"""
CONTROL DE FP v9.9 - PLL con objetivo FP = 0.05 (PHI = ±87.134°)
================================================================

Cambios respecto a v9.8:
  * Objetivo FP = 0.050 → PHI = ±87.134°   (arccos(0.05))
  * Zona EXTREMA: prácticamente en el vértice ±90° donde P = 0.05·S.
    d(cos θ)/dθ = -sin(87.134°) ≈ -0.9987 ⇒ 1° de PHI ≈ 0.0174 de FP.
  * Kp/Ki reducidos todavía más: la relación señal/ruido de P es baja.
  * Umbrales de histéresis/lock más estrechos.
  * Pasos del BUSCAR más finos.
  * Filtrado adicional del WT: al bajar la tasa el ruido de P se atenúa.

Cambios heredados (v9.5–v9.8):
...
⚠ ADVERTENCIA CRÍTICA para FP = 0.05:
  Estamos a sólo 2.87° del vértice ±90° donde P=0 y el signo de la
  potencia activa se invierte. Pequeños errores de medición o desfases
  internos del WT (phase compensation, delays de canal) pueden hacer
  que el FP medido salte ±0.05 sin que el control haga nada. Revisar:
    - Compensación de fase del WT3000 (phase correction).
    - Sincronización (sample rate, zero-crossing) con la señal del FG.
    - Filtro de promediado (update rate más lento).
  Si el WT no es capaz de resolver establemente el ángulo en esta zona,
  el PLL oscilará aunque el algoritmo sea correcto.
"""
import time
import numpy as np
...
# --- Objetivo: FP = 0.050 → PHI = ±87.134° ---
FP_OBJETIVO = 0.050
PHI_OBJETIVO = 87.134          # grados — cos(87.134°) = 0.050

FP_MIN_RANGO = 0.020
FP_MAX_RANGO = 0.080
FP_TIGHT     = 0.050
FP_TIGHT_TOL = 0.020           # |FP - 0.05| < 0.02 → [0.03, 0.07]
...
PHI_BUSCAR_ENTER = 6.0        # |phi_err| > esto → BUSCAR (3 usables)
PHI_BUSCAR_EXIT  = 2.5        # |phi_err| < esto → PLL (2 usables)
N_FRESH_ENTER_BUSCAR = 3
N_FRESH_EXIT_BUSCAR  = 2

# BUSCAR — paso adaptativo (aún más fino por cercanía al vértice)
PASO_BUSCAR_INICIAL   = 8.0     # barrido ciego
PASO_BUSCAR_MAX       = 15.0    # tope absoluto
PASO_BUSCAR_FAR       = 4.0     # paso mínimo cuando |phi_err| > umbral
PASO_BUSCAR_NEAR      = 0.4     # paso mínimo cuando |phi_err| <= umbral
PHI_ERR_FAR_THRESH    = 8.0     # umbral lejos/cerca (grados)
N_SETTLE_BUSCAR = 6
...
STALE_TOL = 0.05
OUTLIER_JUMP = 15.0            # ° — muy bajo por cercanía al vértice

# --- Antídotos contra el bloqueo por stale ---
STALE_REFRESH_LIMIT = 20
N_MAX_WAIT_MOVE     = 25

PHI_LOCKED = 1.5               # |phi_err| < 1.5° → lock
...
class Estadisticas:
    ...
    def registrar(self, fp, phi, fase, frec, df, modo, dt):
        ...
        idx = int(abs(abs(fp) - FP_TIGHT) / 0.02)
        idx = min(idx, 9)
        self.bins_fp[idx] += 1
    ...
    def resumen(self, controlador, t_total, n_iter, en_rango):
        self.cerrar_rachas()
        print("\n" + "=" * 90)
        print(f" RESUMEN FINAL v9.9 — OBJETIVO FP = {FP_OBJETIVO:.3f} "
        ...
        if n_tot > 0:
            ...
            for i in range(9, -1, -1):
                ...
                barra = "#" * int(pct / 2)
                lo = i * 0.02
                print(f"  [{lo:.2f}-{lo+0.02:.2f}] {int(self.bins_fp[i]):5d} "
                      f"({pct:5.1f}%)  {barra}")
        ...
        if self.tiempo_primer_tight is not None:
            print(f"  1er |FP-0.05|<{FP_TIGHT_TOL:.3f}: "
...
def main():
    print("=" * 170)
    print(f" CONTROL FP v9.9 — OBJETIVO FP = {FP_OBJETIVO:.3f} "
          f"(PHI = {PHI_OBJETIVO:.3f}°)  [MODO EXTREME · ZONA CRÍTICA]")
    print("=" * 170)
    ...
    print(f" ⚠⚠  FP=0.05 está a sólo {90.0 - PHI_OBJETIVO:.2f}° del vértice. "
          f"Verificar compensación de fase del WT3000.")
    print("=" * 170)
