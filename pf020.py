"""
CONTROL DE FP v9.7 - PLL con objetivo FP = 0.2 (PHI = ±78.463°)
================================================================

Cambios respecto a v9.6:
  * Paso del BUSCAR adaptativo según |phi_err|:
      - |phi_err| > PHI_ERR_FAR_THRESH → paso mínimo PASO_BUSCAR_FAR
      - |phi_err| <= PHI_ERR_FAR_THRESH → paso mínimo PASO_BUSCAR_NEAR
    Esto elimina la oscilación 65↔50↔65 observada cuando el slope ya
    predice correctamente un paso pequeño pero el código forzaba un
    paso mínimo grande.
  * Se elimina PASO_BUSCAR_MIN (ya no aplica).
  * El paso real se calcula en _buscar() en cada iteración; self.paso_buscar
    queda sólo como referencia informativa para el log.

Cambios heredados de v9.5/v9.6:
  * Objetivo FP = 0.200 → PHI = ±78.463°.
  * Fix stale-block: BUSCAR opera siempre con la última phi conocida;
    refresco forzado y timeout de move_pending.
"""
import time
import numpy as np
...
# --- Objetivo: FP = 0.200 → PHI = ±78.463° ---
FP_OBJETIVO = 0.200
PHI_OBJETIVO = 78.463          # grados — cos(78.463°) = 0.200
...
PHI_BUSCAR_ENTER = 10.0       # |phi_err| > esto → BUSCAR (3 usables)
PHI_BUSCAR_EXIT  = 5.0        # |phi_err| < esto → PLL (2 usables)
N_FRESH_ENTER_BUSCAR = 3
N_FRESH_EXIT_BUSCAR  = 2

# BUSCAR — paso adaptativo
PASO_BUSCAR_INICIAL   = 12.0    # paso en barrido ciego (sin slope fiable)
PASO_BUSCAR_MAX       = 25.0    # tope absoluto de cualquier paso
PASO_BUSCAR_FAR       = 8.0     # paso mínimo cuando |phi_err| > umbral
PASO_BUSCAR_NEAR      = 0.8     # paso mínimo cuando |phi_err| <= umbral
PHI_ERR_FAR_THRESH    = 15.0    # umbral lejos/cerca (grados)
N_SETTLE_BUSCAR = 6
...
class Estadisticas:
    def __init__(self):
        ...
        self.run_phi_lock = RunTracker(
            f"|PHI - {PHI_OBJETIVO:.2f}°| < {PHI_LOCKED:.0f}°")
    ...
    def resumen(self, controlador, t_total, n_iter, en_rango):
        self.cerrar_rachas()
        print("\n" + "=" * 90)
        print(f" RESUMEN FINAL v9.7 — OBJETIVO FP = {FP_OBJETIVO:.3f} "
        ...
        if self.tiempo_primer_tight is not None:
            print(f"  1er |FP-0.2|<{FP_TIGHT_TOL:.3f}: "
...
class ControladorFPv9:
    def __init__(self):
        ...
        # Búsqueda (sólo informativo; el paso real se calcula en _buscar)
    ...
    def _buscar(self, fresh):
        ...
        if self.slope is not None and abs(self.slope) > SLOPE_MIN_ABS:
            ...
            # Paso mínimo adaptativo: cerca del objetivo confiamos en el
            # slope y permitimos pasos muy pequeños; lejos forzamos un paso
            # mayor para escapar de mesetas.
...
def main():
    print("=" * 170)
    print(f" CONTROL FP v9.7 — OBJETIVO FP = {FP_OBJETIVO:.3f} "
