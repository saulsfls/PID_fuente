"""CONTROL DE FP v10.6-0.2 - FP=0.2 con outlier filter arreglado y BUSCAR que sabe parar"""
import time
import numpy as np
...
# v10.6: exit mucho más fácil — no exigimos 4 frescas, con 2 alcanza
...
# v10.6: PLL más suave para no saturar Δf
KP_PLL, KI_PLL = 0.008, 0.0010
DF_MAX, DF_LP = 0.15, 0.45

# v10.5: frescas forzadas y slope post-settling
STALE_TOL          = 0.15
STALE_FORCE_SEC    = 0.6
EFF_DT_MAX         = 1.5
SLOPE_SETTLE_SEC   = 0.30

# v10.6: escape hatch del outlier filter
MAX_OUTLIERS_CONSEC = 5   # tras 5 outliers seguidos, forzar aceptación

# ---- Trim ----
PHI_TRIM_DEADBAND     = 3.0
PHI_TRIM_MAX          = 12.0
PHI_TRIM_MIN          = 0.5
...
# v10.6: slope inicial refleja la planta real (|slope| ≈ 0.7)
SLOPE_INIT, SLOPE_MIN_ABS, SLOPE_SAMPLES_INIT = -0.7, 0.20, 20
OUTLIER_JUMP, PHI_LOCKED = 35.0, 2.0

# ---- Red de seguridad ----
USAR_STUCK_FREQ_CHECK = False
MIN_FRESH_SINCE_LAST  = 15.0
STUCK_FREQ_WINDOW     = 100
STUCK_FREQ_TOL        = 1e-5
HEALTH_PRINT_PERIOD   = 10.0
HEALTH_WARMUP         = 15.0
...
class HealthMonitor:
    ...
    def status(self):
        ...
        self.stuck_freq = False
        if USAR_STUCK_FREQ_CHECK and len(self.freq_window) >= STUCK_FREQ_WINDOW:
    ...
    def maybe_print(self, extra=""):
        ...
        if not healthy:
            print(f"  [!] SALUD: {' | '.join(reasons)}{extra}")
        else:
            std = np.std(self.freq_window) if self.freq_window else 0.0
...
class Estadisticas:
    def __init__(self):
        ...
        self.run_phi_lock = RunTracker(f"|φ - φ_obj| < {PHI_LOCKED:.1f}°")
        self.run_phi_tight = RunTracker(
            f"|φ - φ_obj| < {PHI_TIGHT:.2f}° (equiv. FP tight)")
        self.run_out = RunTracker("FP fuera de rango")
        self.tiempo_primer_lock = self.tiempo_primer_tight = None
        self.intervalos_fresh = []
    ...
    def registrar(self, fp, phi, phi_err, fase, frec, df, modo, dt):
        ...
        self.run_phi_tight.update(abs(phi_err) < PHI_TIGHT, dt)
        self.run_out.update(not in_range, dt)

    def cerrar_rachas(self):
        for r in (self.run_fp_range, self.run_fp_tight, self.run_phi_lock,
                  self.run_phi_tight, self.run_out):
            r.close()
    ...
    def resumen(self, controlador, t_total, n_iter, en_rango):
        self.cerrar_rachas()
        print("\n" + "=" * 90)
        print(" RESUMEN FINAL v10.6-0.2 — Control hacia FP=0.2 (φ_obj = "
              f"{PHI_OBJETIVO:+.2f}°)")
        print("=" * 90)
        print(f"\n[ Tiempo y muestras ]")
        ...
        if self.intervalos_fresh:
            print(f"\n[ Ritmo de datos ]")
            print(f"  Intervalo entre frescas: prom={np.mean(self.intervalos_fresh):.3f}s  "
                  f"mediana={np.median(self.intervalos_fresh):.3f}s  "
                  f"max={np.max(self.intervalos_fresh):.3f}s")
            print(f"  Tasa frescas efectiva:   "
                  f"{1/max(np.mean(self.intervalos_fresh),1e-3):.2f} Hz")
        ...
        if self.tiempo_primer_lock is not None:
            print(f"  1er lock (|Δφ|<{PHI_LOCKED:.1f}°): {self.tiempo_primer_lock:.2f} s")
        ...
        print(f"  Trim: deadband={PHI_TRIM_DEADBAND}°  max={PHI_TRIM_MAX}°  "
              f"min={PHI_TRIM_MIN}°  cooldown={N_FRESH_COOLDOWN_TRIM}")
        print(f"  v10.6: MAX_OUTLIERS_CONSEC={MAX_OUTLIERS_CONSEC}  "
              f"BUSCAR_EXIT<{PHI_BUSCAR_EXIT}°x{N_FRESH_EXIT_BUSCAR}  "
              f"SLOPE_INIT={SLOPE_INIT}")
        print(f"  STUCK_FREQ check: {'ON' if USAR_STUCK_FREQ_CHECK else 'OFF'}")
        if self.df_hist:
        ...
        if n_iter > 0:
            ...
            print(f"  {v} ({ef:.1f}% en banda FP≈0.2)")
        print("=" * 90)


# ------------------------------------------------------- CONTROLADOR v10.6-0.2
class ControladorFPv10:
    """v10.6-0.2: outlier filter contra muestra anterior, escape hatch,
    BUSCAR con parada proporcional, PLL más suave."""
    ...
    def _actualizar_phi(self, phi_raw_deg, stats=None):
        """v10.6: outlier contra muestra anterior + escape hatch + force_fresh
        que vence al filtro de outlier."""
        ...
        # ¿Cuánto saltó respecto a la MUESTRA ANTERIOR?
        ...
        if is_outlier:
            ...
            # Escape hatch: demasiados outliers seguidos → aceptar como baseline
            if self.n_outliers_consec < MAX_OUTLIERS_CONSEC:
                self.phi_prev_raw = phi_w
                self.phi_stale = False
                return False
            # si llegamos aquí: forzar aceptación (posible cambio real de régimen)
        else:
            self.n_outliers_consec = 0

        # Actualizamos prev_raw SIEMPRE (para que el próximo outlier se mida bien)
        self.phi_prev_raw = phi_w

        # ¿Cuánto cambió respecto a la última fresca aceptada?
        step_fresh = abs(envolver_fase(phi_w - self.phi_fresh))
        force_fresh = (now - self.last_fresh_time) > STALE_FORCE_SEC

        # Decisión de "fresh":
        #   - si fue outlier, ya decidimos aceptar (escape hatch)
        #   - si no fue outlier y no forzamos y el cambio es pequeño → stale
    ...
    def _buscar(self, fresh):
        ...
        if self.slope is not None and abs(self.slope) > SLOPE_MIN_ABS:
            ...
            # v10.6: sólo forzamos paso mínimo si estamos LEJOS del objetivo.
            # Si estamos cerca, usamos corrección proporcional (permite que
            # se acumulen frescas con |err| < PHI_BUSCAR_EXIT para pasar a PLL).
            if abs(err) > PHI_BUSCAR_EXIT:
                delta = np.sign(delta) * max(abs(delta), PASO_BUSCAR_MIN)
        else:
            delta = self.dir_buscar * self.paso_buscar
        delta = clamp(delta, -PASO_BUSCAR_MAX, PASO_BUSCAR_MAX)
        cambio = self._mover_fase(delta)
        return self._resp(f"BUSCAR φ{delta:+.1f}°", FREC_NOMINAL,
                          cambio_fase=cambio, fresh=True)
...
def main():
    print("=" * 175)
    print(" CONTROL FP v10.6-0.2 - FP=0.2 + outlier fix + BUSCAR que sabe parar  "
          f"(φ_obj = {PHI_OBJETIVO:+.2f}°)")
    print("=" * 175)
    print(f" Muestreo objetivo: {1/INTERVALO_MUESTREO:.0f} Hz")
    print(f" Kp={KP_PLL:.5f} Ki={KI_PLL:.6f} |Δf|max={DF_MAX:.3f} Hz  DF_LP={DF_LP}")
    print(f" φ_objetivo={PHI_OBJETIVO:+.2f}°  ⇒  FP_objetivo≈{FP_OBJETIVO:.3f}")
    print(f" Banda FP: [{FP_MIN_RANGO:.2f}, {FP_MAX_RANGO:.2f}]  "
          f"tight [{FP_TIGHT_LOW:.2f}, {FP_TIGHT_HIGH:.2f}]")
    print(f" Trim: deadband={PHI_TRIM_DEADBAND}°  max={PHI_TRIM_MAX}°  "
          f"min={PHI_TRIM_MIN}°  cooldown={N_FRESH_COOLDOWN_TRIM}")
    print(f" v10.6: MAX_OUTLIERS_CONSEC={MAX_OUTLIERS_CONSEC}  "
          f"BUSCAR_EXIT: <{PHI_BUSCAR_EXIT}° x {N_FRESH_EXIT_BUSCAR} frescas")
    print(f" v10.5: STALE_TOL={STALE_TOL}°  STALE_FORCE={STALE_FORCE_SEC}s  "
          f"EFF_DT_MAX={EFF_DT_MAX}s  SLOPE_SETTLE={SLOPE_SETTLE_SEC}s")
    print(f" Detector frecuencia congelada: "
          f"{'ON' if USAR_STUCK_FREQ_CHECK else 'OFF'}")
    print("=" * 175)
    ...
    try:
        ...
        while time.time() - t_ctrl < TIEMPO_PRUEBA_SEG:
            ...
            if res.get("fresh"):
                if stats._last_fresh_t is not None:
                    stats.intervalos_fresh.append(t_now - stats._last_fresh_t)
                stats._last_fresh_t = t_now
