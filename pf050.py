"""
CONTROL DE FP v9.5 - PLL con objetivo FP = 0.5 (PHI = ±60°)
================================================================

Cambios respecto a v9.4:
  * FIX del bloqueo por detección de "stale":
      - _buscar() ahora opera SIEMPRE con la última phi conocida, no
        sólo cuando la lectura es "fresca". Un sistema físicamente estable
        devuelve el mismo PHI y antes se interpretaba como stale → el
        controlador quedaba congelado sin reaccionar.
      - Refresco forzado tras STALE_REFRESH_LIMIT lecturas repetidas
        consecutivas cuando no hay movimiento pendiente.
      - Timeout del move_pending (N_MAX_WAIT_MOVE): si llevamos demasiado
        tiempo esperando una respuesta a un movimiento de fase, lo damos
        por perdido y desbloqueamos.
      - La histéresis BUSCAR↔PLL también cuenta lecturas stale "refrescadas"
        para no quedar atrapada en BUSCAR.

Cambios heredados de v9.4:
  El objetivo no es PHI = 0 (FP = 1) sino PHI = ±60° (cos 60° = 0.5).
      phi_err = envolver_fase(phi - PHI_OBJETIVO)
      err     = -phi_err / slope
"""
import time
import numpy as np
...
# --- Objetivo: FP = 0.5 → PHI = ±60° ---
FP_OBJETIVO = 0.500
PHI_OBJETIVO = 60.0            # grados — cos(60°) = 0.5

FP_MIN_RANGO = 0.470
FP_MAX_RANGO = 0.530
FP_TIGHT     = 0.500           # valor objetivo
FP_TIGHT_TOL = 0.010           # |FP - FP_TIGHT| < tol
...
PHI_BUSCAR_ENTER = 50.0       # |phi_err| > esto → BUSCAR (3 frescas/usable)
PHI_BUSCAR_EXIT  = 25.0       # |phi_err| < esto → PLL (2 frescas/usable)
...
STALE_REFRESH_LIMIT = 20       # refrescos forzados tras N stales sin move pendiente
N_MAX_WAIT_MOVE     = 25       # iteraciones máx. esperando respuesta a un move

PHI_LOCKED = 5.0               # |phi_err| < 5° → lock
...
class Estadisticas:
    def __init__(self):
        ...
        self.run_phi_lock = RunTracker(
            f"|PHI - {PHI_OBJETIVO:.0f}°| < {PHI_LOCKED:.0f}°")
    ...
    def resumen(self, controlador, t_total, n_iter, en_rango):
        self.cerrar_rachas()
        print("\n" + "=" * 90)
        print(f" RESUMEN FINAL v9.5 — OBJETIVO FP = {FP_OBJETIVO:.3f} "
              f"(PHI = {PHI_OBJETIVO:.1f}°)")
        print("=" * 90)
        ...
        if self.tiempo_primer_lock is not None:
            print(f"  1er lock (|PHI-obj|<5°): {self.tiempo_primer_lock:.2f} s")
        if self.tiempo_primer_tight is not None:
            print(f"  1er |FP-0.5|<{FP_TIGHT_TOL:.3f}: "
...
class ControladorFPv9:
    ...
    def _actualizar_phi(self, phi_raw_deg, stats=None):
        ...
        Distingue entre 'stale real' (el instrumento aún no ha actualizado
        tras un movimiento) y 'estable' (sistema físicamente quieto que
        repite la misma lectura). Sólo el primero debe bloquear el control.
        """
        ...
        if not changed:
            # Valor repetido: candidato a stale.
            self.stale_run += 1
            self.phi_stale = True

            # Refresco forzado si NO esperamos respuesta a un movimiento:
            # el sistema simplemente está estable y hay que reaccionar.
            ...
            # Timeout del move_pending: si llevamos demasiado tiempo
            # esperando una respuesta que no llega, asumimos que el
            # instrumento NO va a actualizar y desbloqueamos el control.
            if self.move_pending and (self.stale_run >= N_MAX_WAIT_MOVE):
                self.phi_fresh = phi_w            # último valor conocido
                self.phi_prev_raw = phi_w
                self.move_pending = False
                self.fresh_after_move = False
                self.phi_stale = False
    ...
    def actualizar(self, fp_med, phi_med, f_med, dt, stats=None):
        ...
        # Distancia angular al objetivo (0 = perfecto)
        phi_err = phi_err_de(self.phi_fresh)
        ...
        # --- Histéresis BUSCAR↔PLL ---
        # Contamos sobre "usable": lecturas frescas o stales refrescadas
        # (stale_run == 0 y sin move pendiente). Así la histéresis no se
        # queda atrapada en BUSCAR cuando el sistema está estable.
    ...
    def _buscar(self, fresh):
        # En BUSCAR operamos SIEMPRE con la última phi conocida (fresh o no).
        # El único bloqueo legítimo es:
        #   (a) settle tras un movimiento reciente, o
        #   (b) esperar la respuesta de un movimiento pendiente (con timeout
        #       gestionado ya en _actualizar_phi).
        ...
        # En BUSCAR, Δf = 0 (la fase del FG barre libremente)
        self.delta_f = 0.0

        phi_err = phi_err_de(self.phi_fresh)

        if self.slope is not None and abs(self.slope) > SLOPE_MIN_ABS:
            # d(PHI) = slope · dφ  ⇒  dφ = -phi_err / slope para llevar
            # phi_err hacia 0.
            delta = -phi_err / self.slope
            # Garantizamos un paso mínimo para no quedarnos clavados.
...
def main():
    print("=" * 170)
    print(f" CONTROL FP v9.5 — OBJETIVO FP = {FP_OBJETIVO:.3f} "
          f"(PHI = {PHI_OBJETIVO:.1f}°)  [MODO EXTREME]  ·  FIX stale-block")
    print("=" * 170)
